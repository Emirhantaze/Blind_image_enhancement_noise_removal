...
scalerY = scalerY.fit(np.reshape(y_train, (len(y_train), 1)))

X_train = scalerX.transform(X_train)
X_test = scalerX.transform(X_test)

y_train = scalerY.transform(np.reshape(y_train, (len(y_train), 1)))
y_test = scalerY.transform(np.reshape(y_test, (len(y_test), 1)))
...
), 'Ada Boosting Regression': ABR(), 'K-Neighbors Regression': KNR(), 'Support Vector Regression': SVR(), 'Ridge Regression': RR()}
pred = []
print(models.keys())
...
final_pred = final.predict(X_train)


sns.distplot(y_train, hist=False, label='Actual')
